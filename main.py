...
from brain import get_reply, reload_knowledge, _load_knowledge, _prefetch_websites
...
async def startup_event():
    """Pre-warm knowledge base and website cache on server startup."""
    try:
        _load_knowledge()
        logger.info("Knowledge base pre-warmed on startup")
    except Exception as e:
        logger.warning(f"Startup knowledge pre-warm failed: {e}")
    try:
        _prefetch_websites()
        logger.info("Website prefetch triggered on startup")
    except Exception as e:
        logger.warning(f"Startup website prefetch failed: {e}")
...
async def api_stream_handler(request: dict):
    """Streaming TTS endpoint - sends text first, then streams audio chunks for low latency"""
    cors_headers = {
        "Access-Control-Allow-Origin": "*",
